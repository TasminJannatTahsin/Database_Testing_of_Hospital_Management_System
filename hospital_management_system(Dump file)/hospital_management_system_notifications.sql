...
CREATE TABLE `notifications` (
  ...
  CONSTRAINT `chk_notifications_read` CHECK ((((`is_read` = false) and (`read_at` is null)) or ((`is_read` = true) and (`read_at` is not null)))),
  CONSTRAINT `chk_notifications_status` CHECK ((`status` in (_cp850'Active',_cp850'Archived')))
